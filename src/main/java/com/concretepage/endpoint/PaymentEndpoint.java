...
//get details by id
	...
	public Response addPaymentDetails(PaymentDetails paymentdetails) {
        ...
        return Response.created(URI.create("/Electro-grid/payment/"+ paymentdetails.getPaymentId())).entity("Payment Add Successfully!!!").build();
	}	
	@PUT
	@Path("/update")
	@Produces(MediaType.APPLICATION_JSON)
	...
	public Response updatePaymentDetails(PaymentDetails paymentdetails) {
		...
		return Response.ok(paymentdetails).entity("Payment Detail Update Successfully!!!").build();
	}
	@DELETE
	@Path("/{id}")
	@Consumes(MediaType.APPLICATION_JSON)		
